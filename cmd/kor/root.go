...
func addFilterOptionsFlag(cmd *cobra.Command, opts *filters.Options) {
	...
	rootCmd.PersistentFlags().StringSliceVar(&opts.IncludeCrds, "include-crds", opts.IncludeCrds, "Custom resources defintions to search, split by commas. Example: --include-crds argo-rollouts. If set, the chosen crd will be returned (in addition to the other resources, of course)")
}
